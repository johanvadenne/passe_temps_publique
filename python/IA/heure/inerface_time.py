...
def actionEvent(event):     
    ...
    # retourne l'heure
    def heure():
        text_area.insert(INSERT, "il est "+ strftime("%H:%M:%S"))
    ...
    if probabilite >= 50 or (pointQuestion2 > 6):
        ...
        text_area.insert(INSERT, "désolé je ne comprend pas votre question")
...
lblq = Label(root, text = "question")
entry.bind("<Return>", actionEvent)
lbl = Label(root, text = "sortie:")
text_area = ScrolledText(wrap = WORD, width = 40, height = 10, font = ("Times New Roman", 11))
lblq.pack() 
entry.pack() 
lbl.pack() 
text_area.pack()
